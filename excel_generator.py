...
def get_catalog():
    """Retourne les listes de panneaux, onduleurs, batteries utilisées à la fois
    dans l'Excel et dans l'app Streamlit."""
    panels = [
...
def generate_workbook_bytes(config: dict) -> bytes:
    """
    Génère le classeur Excel complet et renvoie les bytes pour téléchargement.

    config doit contenir :
      - panel_id (str)
      - n_modules (int)
      - grid_type (str: "Mono" / "Tri 3x400")
      - battery_enabled (bool)
      - battery_kwh (float)
      - annual_consumption (float)
      - consumption_profile (str: "Standard","Hiver fort","Été fort")
    Les valeurs seront pré-remplies dans les onglets Choix / Profil / Synthese.
    """
    ...
    # Préremplissage avec les choix de l'utilisateur
    ws_choice["B1"] = config.get("panel_id", panels[0][0])
    ws_choice["B2"] = config.get("n_modules", 10)
    ws_choice["B3"] = config.get("grid_type", "Mono")
    ws_choice["B4"] = "Oui" if config.get("battery_enabled", False) else "Non"
    ws_choice["B5"] = config.get("battery_kwh", 6)
    ws_choice["B6"] = config.get("max_dc_ac", 1.3)

    # Menus déroulants
    dv_panel = DataValidation(
        type="list",
    ...
    for offset in range(len(inverters)):
        ...
        ws_choice[f"F{row}"] = (
            f"=IF(D{row}=\"\",\"\",IF(D{row}<=$B$6,\"OK\",\"NO\"))"
        )
        ws_choice[f"G{row}"] = (
            f"=IF(AND(E{row}=\"OK\",F{row}=\"OK\"),\"OK\",\"NO\")"
        )
        ws_choice[f"H{row}"] = (
            f"=IF($B$4<>\"Oui\",\"Aucune\",IF($B$5<=6,\"Sigen6\",\"Sigen10\"))"
        )
        row += 1

    _autofit(ws_choice, max_col=8)

    # ========= PROFIL =========
    ws_prof = wb.create_sheet("Profil")
    ws_prof["A1"] = "Conso annuelle (kWh)"
    ws_prof["B1"] = config.get("annual_consumption", 3500)
    ws_prof["A2"] = "Profil conso"
    ws_prof["B2"] = config.get("consumption_profile", "Standard")

    dv_prof = DataValidation(
        type="list", formula1="\"Standard,Hiver fort,Été fort\""
    )
    ...
    ws_prof.append([""])
    ws_prof.append(
        ["Mois", "%_conso", "Conso_kWh", "Prod_PV_kWh", "kWh_kWp_BEL", "Autocons_kWh"]
    )
    ...
    data_bar = Reference(
        ws_prof, min_col=3, max_col=4, min_row=4, max_row=4 + len(months)
    )
    cats = Reference(
        ws_prof, min_col=1, min_row=5, max_row=5 + len(months) - 1
    )
    ...
    ws_str["B5"] = config.get("t_min", -10)
    ws_str["B6"] = config.get("t_max", 70)
    ws_str["B7"] = config.get("n_series", 10)

    dv_p2 = DataValidation(
        type="list",
        formula1=f"=Catalogue!$A${first_panel_row}:$A${last_panel_row}",
    )
    dv_i2 = DataValidation(
        type="list",
    ...
    ws_str["B17"] = (
        "=IF(OR(B9=\"\",B11=\"\"),\"\",B7*B9*(1+B11/100*(B5-25)))"
    )
    ws_str["A18"] = "Vmp_string_chaud (V)"
    ws_str["B18"] = (
        "=IF(OR(B10=\"\",B11=\"\"),\"\",B7*B10*(1+B11/100*(B6-25)))"
    )

    ws_str["A20"] = "Check Voc <= V_DC_max"
    ws_str["B20"] = (
        "=IF(OR(B17=\"\",B13=\"\"),\"\",IF(B17<=B13,\"OK\",\"DÉPASSE\"))"
    )
    ws_str["A21"] = "Check Vmp dans plage MPPT"
    ws_str["B21"] = (
        "=IF(OR(B18=\"\",B14=\"\",B15\"\"),\"\",IF(AND(B18>=B14,B18<=B15),\"OK\",\"HORS PLAGE\"))"
    )

    _autofit(ws_str, max_col=4)
    ...
    ws_h["B2"] = (
        "=IF(OR($B$1<1,$B$1>12),\"\","
        "INDEX(Profil!$C$5:$C$16,$B$1)"
        "/INDEX({31;28;31;30;31;30;31;31;30;31;30;31},$B$1))"
    )
    ws_h["A3"] = "Prod jour (kWh)"
    ws_h["B3"] = (
        "=IF(OR($B$1<1,$B$1>12),\"\","
        "INDEX(Profil!$D$5:$D$16,$B$1)"
        "/INDEX({31;28;31;30;31;30;31;31;30;31;30;31},$B$1))"
    )
    ...
    for h in range(24):
        ...
        ws_h.cell(r, 2).value = "=IF($B$2=\"\",\"\",$B$2/24)"
        ws_h.cell(r, 3).value = f"=IF($B$3=\"\",\"\",$B$3*{pv_frac[h]})"
        ws_h.cell(
            r, 4
        ).value = f"=IF(OR(B{r}=\"\",C{r}=\"\"),\"\",MIN(B{r},C{r}))"

    _autofit(ws_h, max_col=5)
    ...
    ws_syn["B18"] = (
        "=IF(B16<>\"Oui\",\"Aucune\",IF(Choix!B5<=6,\"Sigen6\",\"Sigen10\"))"
    )

    _autofit(ws_syn, max_col=4)
